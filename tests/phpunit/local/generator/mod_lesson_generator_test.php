...
final class mod_lesson_generator_test extends \advanced_testcase {
    ...
    public function test_create_activity(): void {
        ...
        $course = $generator->core_course->create_course(['category' => $category->id, 'shortname' => 'LEGT1']);

        $lesson = $generator->mod_lesson->create_activity([
            'course' => $course,
            'name' => 'Test Lesson',
        ]);

        $this->assertSame('Test Lesson', $lesson->name);
        $this->assertObjectHasProperty('cmid', $lesson);
        $this->assertTrue($DB->record_exists('lesson', ['id' => $lesson->id]));
    }
}
